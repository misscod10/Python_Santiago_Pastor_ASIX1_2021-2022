...
__license__ = "GPL3"
__becario_de_turno__ = "Santiago Pastor Serrano"
...
def get_textos(path, idioma):
        ...
        for i in range(len(dades)):
            if dades[i]["codi"]==idioma:
                text=dades[i]["textos"]
                file.close()
                return text
        raise Exception("Llenguatge no suportat, torna a intentar-ho.")
...
def canvia_idioma():
    global textos_multilingues, IDIOMES
    mostra_posibles_idiomes()
    idioma_escollit=input("Introdueix l'idioma a utilitzar: ")
    textos_multilingues = get_textos(PATH_TEXTOS, idioma_escollit)
